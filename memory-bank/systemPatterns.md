...
- **Lifecycle governance:** controlling factory manager of the same factory sets the calendar while `approved`, writes phases **and** tasks (planning in `approved`, Kanban after start), starts/pauses/resumes, and requests completion. Controlling director confirms completion and may pause/resume. After approval, budget/duration/code freeze; start/end dates freeze once execution starts. Funding write = controlling director + FM after approval. Procurement/staff/overhead write = controlling FM after approval. Escalations are blocked tasks with `escalation_status` + `acknowledge_task_escalation`. `assigned_pm_id` remains on `projects` but is unused.
- **Write vs view:** `profiles.can_control` + `auth_can_control()`; SPA helpers in `src/lib/roles.ts` (`canControl`). Viewers keep role-scoped SELECT; they cannot comment, acknowledge, import, or open accounts/factories.
