...
describe('execution readiness', () => {
  ...
  it('is ready without an assigned project manager', () => {
    const ready = getExecutionReadiness(baseProject, [
      {
        start_date: '2026-01-01',
        end_date: '2026-02-15',
    ...
    expect(ready.ready).toBe(true)
    expect(ready.reasons).not.toContain('missing_assigned_pm')
  })
  ...
  it('allows only controlling factory manager of the same factory to start', () => {
    expect(
      canStartExecution(
  ...
  it('lets only the controlling factory manager of the same factory write phases', () => {
    const project = {
      status: 'approved' as const,
      factory_id: 'f1',
    }
